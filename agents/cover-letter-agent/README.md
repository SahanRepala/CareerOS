...
Drafts a cover letter grounded in the resume and job description.

## Contract

Implements `Agent<CoverLetterAgentInput, CoverLetterAgentOutput>` from `/contracts/agent.contract.ts`.
...
1. Replace the `createPlaceholderAgent(...)` call in `index.ts` with a real
   `Agent<CoverLetterAgentInput, CoverLetterAgentOutput>` implementation (a hand-written object or class
   is fine — the contract doesn't care).
2. Add prompt content to `/prompts/cover-letter-agent.md`.
